CREATE PROCEDURE summarize_offlinx_visit_revenue_2 @FromDate DATE, @ToDate DATE
AS
...
+ CONVERT(VARCHAR, SUM( CAST((attribution_amount * IIF(purchase_method = 'E', 1, otvc.factor)) AS INTEGER ) )) + '","'
+ CONVERT(VARCHAR, CAST( ROUND( SUM( IIF(purchase_method = 'E', 1, otvc.factor) / total_attributions ) , 0) AS INTEGER) ) + '","'
--+ CONVERT(VARCHAR, CAST( ROUND((COUNT(DISTINCT visitor_id) /* + SUM(iif(visitor_id is null, 1, 0))*/ ) * AVG( IIF(purchase_method = 'E', 1, otvc.factor) ),0) AS INTEGER)) + '"]'
+ CONVERT(VARCHAR, CAST( CAST(COUNT(DISTINCT visitor_id) AS DECIMAL) / CAST(COUNT(DISTINCT oae.offlinx_transaction_id) AS DECIMAL) * ROUND( SUM( IIF(purchase_method = 'E', 1, otvc.factor) / total_attributions ),0) AS INTEGER)) + '"]'
AS revenue

FROM 
offlinx_attribution_expanded oae
...
WHERE oae.site_transaction_date BETWEEN @FromDate AND @ToDate
GROUP BY
oae.offlinx_site_id,
desktop,
mobile,
